...
| **API クレジット消費への配慮** | `zoho.crm.coql` にデータアクセスを集約。バックエンド呼び出し 1 回あたり最大 2,000 件（COQL のハード上限）を取得し、全件を React State（チャンク）に保持。UI は in-memory チャンクを 100 件/ページでスライス表示 — チャンク内のページ操作はバックエンド呼び出しなし・クレジット消費なし。チャンク内の最終レコードを超えたページへの移動時のみ次のチャンクを取得。バックエンド呼び出しは「レポート生成」ボタン押下時のみ（オンデマンド実行・自動更新なし）。 |
...
7. `zoho.crm.coql` で COQL を実行し、1 回の呼び出しで最大 2,000 件（1 チャンク）を取得する。
8. チャンク全体を React State（`reportData`）に保存する。UI は in-memory 配列を 100 件/ページでスライス表示 — 同一チャンク内のページ操作はバックエンド呼び出しなし。ユーザーがチャンク内の最終レコードを超えたページへ移動した場合のみ、次の 2,000 件チャンクを取得する新しいバックエンド呼び出しが発火する（`OFFSET` を 2,000 進める）。
...
| Zoho CRM COQL | CRM データのクエリ; ドット記法による Lookup 項目アクセス | Deluge `zoho.crm.coql`（推奨; バックエンド呼び出し 1 回あたり最大 2,000 件; UI は in-memory チャンク内でページング） | Zoho API クレジット上限の対象 |
...
| COQL クレジット枯渇（大量クエリによる DoS） | バックエンド呼び出し 1 回で最大 2,000 件を取得（1 クレジット）; UI は React State から後続ページを提供し追加の COQL 呼び出しなし。Deluge がクエリあたり最大 2 Lookup トラバーサルを強制（COQL 公式制限）。フロントエンドが実行中は実行ボタンを無効化。 |
...
- **`zoho.crm.coql` を唯一のクエリ API として使用**: すべてのデータ取得を `zoho.crm.coql`（バックエンド呼び出し 1 回あたり最大 2,000 件、COQL ハード上限）に集約し、標準検索 API を使用しない。N+1 フェッチパターンを排除し、1 回のクレジット消費呼び出しから提供できる UI ページ数を最大化する。
- **チャンクベース React State キャッシュ + オンデマンド実行**: バックエンド呼び出し 1 回で最大 2,000 件のチャンクを `reportData` React State に格納する。UI はこの配列を 100 件/ページでスライス — チャンク内のページ操作はクレジット消費なし・バックエンド呼び出しなし。現在のチャンクの最終レコードを超えた移動時のみ次のチャンク取得（新規バックエンド呼び出し）が発火する。最初のバックエンド呼び出しは「レポート生成」を明示的にクリックした場合のみ実行される。
...
| 指標 | 現在の推定値 | 限界点 |
|---|---|---|
| 同時接続ウィジェットユーザー | アプリレベルの制限なし | Zoho プラットフォームが同時負荷をネイティブに処理。アプリレベルのエラー閾値なし。Zoho が 429 を返した場合、フロントエンドはローディングスピナー状態を維持し 30 秒後に自動リトライ（最大 3 回・サイレント） — エラーメッセージは表示しない。ユーザーはエラーではなく遅延を体感する。 |
| バックエンドフェッチあたりのレコード数 | COQL 呼び出し 1 回あたり最大 2,000 件 | COQL ハード上限; UI は in-memory チャンクを 100 件/ページでスライス。次のチャンクは現在のチャンクが枯渇した場合のみ取得。 |
...
- **全件スキャンよりチャンクフェッチ**: 単一クエリで全件取得しない。COQL 呼び出し 1 回あたり最大 2,000 件（`LIMIT 2000 OFFSET (chunk * 2000)`）を取得する。UI は in-memory チャンクを 100 件/ページでページング。チャンク境界を跨ぐ際はローディングインジケーターを表示する。
- **クレジット予算**: COQL 呼び出し 1 回あたり Zoho API クレジットを 1 消費。チャンク内のページ操作は追加クレジット消費なし。次チャンクの取得はユーザーの明示的な操作（ページ操作でチャンク境界を超えた時）のみ実行（自動全件取得なし）。
