...
| `TOO_MANY_REQUESTS` | Zoho API が HTTP 429（レート制限超過）を返す; フロントエンドはローディングスピナー状態を維持し 30 秒後に自動リトライ（最大 3 回） — ユーザーにエラーメッセージは表示しない |
...
| `TOO_MANY_REQUESTS` ハンドリング | `zoho.crm.coql` が 429 エラーを返した場合、Deluge はフロントエンドに `TOO_MANY_REQUESTS` を返す。フロントエンドは**ローディングスピナー状態を維持 — エラーメッセージは表示しない**。フロントエンドは 30 秒後に同じ呼び出しを自動リトライし、最大 3 回まで繰り返す。Deluge 側でのリトライループなし（クレジット積み上げを回避）。3 回リトライ後も失敗した場合のみ「処理に時間がかかっています」という柔らかい案内を表示する。ユーザーはエラーではなく遅延を体感する。 |
...
stateDiagram-v2
    ...
    実行中 --> 待機中 : Zoho が 429 を返す（スピナー継続表示）
    実行中 --> エラー : Function がエラーを返す
    待機中 --> 実行中 : 30 秒後に自動リトライ（最大 3 回・サイレント）
    待機中 --> 準備完了 : ユーザーがキャンセル
